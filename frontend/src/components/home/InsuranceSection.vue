<template>
  <div class="page">
    ...
    >
      ...
      <el-table-column prop="startDate" label="起保日期" width="80" />
      <el-table-column prop="endDate" label="结束日期" width="80" />
      <el-table-column v-if="isAdmin" prop="agent" label="业务员" width="70" show-overflow-tooltip />
      <el-table-column label="操作" width="132" fixed="right" align="center">
        <template #default="scope">
          <div class="insurance-actions">
            ...
            <el-button link type="success" @click="$emit('download-policy', scope.row)">导出</el-button>
            <el-button
...
.filter-form-row :deep(.el-input__wrapper) {
  border-radius: 12px;
}
...
.insurance-table :deep(.el-table__cell) {
  padding-top: 8px;
  padding-bottom: 8px;
}

.insurance-table :deep(.el-table__header-wrapper th) {
  background: #f7f9fc;
  color: #1f2937;
  font-weight: 700;
}
...
.insurance-table :deep(.el-table__row:hover > td) {
  background: #f8fbff !important;
}

.insurance-table :deep(.el-tag) {
  border-radius: 999px;
  padding: 0 8px;
  font-weight: 700;
  font-size: 12px;
...
.insurance-table :deep(.el-button.is-link) {
  padding: 0 2px;
  font-weight: 600;
  min-height: auto;
}
...
.insurance-actions :deep(.el-button) {
  margin-left: 0;
}
